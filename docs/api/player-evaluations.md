...
所有接口使用 JSON，并通过登录后获得的 `compintel_session` HttpOnly Cookie 验证用户。游戏必须已经发布。已登录且未封禁的用户可以查看所有用户提交的源码与评测结果；提交时 API 只会创建或复用当前用户自己的 Player。
...
同一用户在同一游戏中首次使用某个名称时创建 Player 和版本 1；再次使用相同名称调用此接口时复用该 Player，并自动创建当前最大版本号加 1 的不可变版本。不同用户可以使用相同名称，一份程序由游戏、用户、Player 名称和版本号共同确定。源码不能为空，最大 256 KiB。每个用户对每个游戏在 24 小时滑动窗口内最多提交 50 次（按新建 `PlayerVersion` 计数），超出返回 `429 SUBMISSION_RATE_LIMIT`。API 为所有已启用平台 Player 的最新 C++ 版本分别创建评测；没有可用平台程序时返回 `503 EVALUATION_OPPONENT_UNAVAILABLE`。
...
公开表示所有已登录且未封禁的用户均可读取，不要求是提交所有者。未发布游戏及平台内置 Player 源码不通过这些接口公开。接口只返回用户 Player 的不可变版本，保证已展示的源码与历史评测严格对应。

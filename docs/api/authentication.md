...
`turnstileToken` 为可选字段：当该客户端 IP 在 3 小时滑动窗口内成功发送验证邮件已超过 5 封时必须提供，并由 Cloudflare Turnstile siteverify 校验。
...
- `POST /v1/auth/verify-email`：请求体为 `{ "username": "alice_01", "code": "123456" }`。校验通过后返回 `{ "user": … }`，用户仍保持 `PENDING` 等待审核。若该用户邮箱已验证，返回 `{ "ok": true }`（不泄露用户资料），不要求验证码。
- `POST /v1/auth/resend-verification`：请求体为 `{ "username": "alice_01", "turnstileToken"?: "…" }`。60 秒内不可重复发送。发信成功后才更新验证码哈希与冷却时间；发信失败保留旧码。成功返回 `{ "ok": true }`。同样受 per-IP Turnstile / 封堵规则约束。
...
客户端 IP 解析优先级：`CF-Connecting-IP` → `X-Real-IP` → `X-Forwarded-For`（最左侧）→ Fastify `req.ip`。计数保存在 Redis（键 TTL 约 3 小时），仅在**成功发送**验证邮件后递增：

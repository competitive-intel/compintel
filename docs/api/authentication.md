...
管理员邮箱默认为 `{username}@compintel.local`，并视为已验证。存量用户迁移同样回填为 `{username}@compintel.local`。生产环境必须覆盖示例密码。未设置 `ADMIN_PASSWORD` 时，seed 会跳过管理员创建；若相同用户名或邮箱已经存在，seed 会保留原记录，不修改显示名、密码、角色、邮箱或验证状态。

...
> 项目已支持手动部署，当前已打通五子棋和路墙棋 MVP 的邮箱验证与账号封禁、游戏目录、Player 提交、多平台对手评测、加权分数和回放闭环；自动部署流水线尚未提供。
...
- 记录对局结果、异常原因、资源摘要、加权评测分数以及五子棋、路墙棋回放
- 提供登录、注册、游戏目录/详情、提交、公开评测、回放和管理页面

当前 Worker 实现五子棋和路墙棋。用户之间的正式 Match 调度、排行榜、Rating 及其变更、对象存储和完整 Playwright E2E 仍未实现；数据库中的 Match 相关模型只是骨架。部署方式请参阅 [DEPLOY.md](DEPLOY.md)。
...
- `packages/logger`：API 与 Worker 共用的结构化日志、级别和敏感字段脱敏
...
同时启动 API、Worker 与 Web SPA：

```bash
pnpm start
```

该命令会通过 Turborepo 启动三个进程，缺少 Worker 时提交只会进入队列而不会开始评测。需要分别调试时，可在三个终端运行：
...
Web SPA 默认访问 `http://localhost:5173`。

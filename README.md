...
> 项目目前处于上线前开发阶段，已打通五子棋 MVP 的账号审核、游戏目录、Player 提交、多平台对手评测、加权分数和回放闭环。
...
当前 Worker 只实现五子棋。用户之间的正式 Match 调度、排行榜、Rating 及其变更、对象存储、生产部署流水线和完整 Playwright E2E 仍未实现；数据库中的 Match 相关模型只是骨架。
